...
def main(settings):
	...
	for t in thresh:
		...
		for episode in range(0, 30000):
			...
			relation_to_score = defaultdict(list)
			for (rule, score) in rule_sim_pairs:
				relation_to_score[rule_to_relation[rule]].append(score.detach().cpu().item())
			
			# another list of (relation, avg score for relation) pairs
			relation_avg_score_pairs = [(r, np.mean(s)) for (r,s) in relation_to_score.items()]
		
			if not relation_avg_score_pairs:
				pred_relations.append(NO_RELATION)
				continue
		
			max_sim = 0.0		
			for pair in relation_avg_score_pairs:
				if(pair[1] > t):
					max_sim = pair[1]
					best_relation = pair[0]
			...
			if max_sim == 0.0:
				...
				pred_relations.append(best_relation)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cache_path', type=str, default="fewshot_tacred/5_way_5_shot/")
